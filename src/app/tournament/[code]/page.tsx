...
import { User, CheckCircle2, Hash, Trophy, Copy, Check, X } from "lucide-react";
...
export default function TournamentLobby({ params }: { params: Promise<{ code: string }> }) {
  ...
  // Public existence check — surfaces "not found" early
  useEffect(() => {
    let cancelled = false;
    fetch(`/api/tournaments/${code}/check`)
      .then((response) => {
        if (cancelled) return;
        if (response.status === 404) setTournamentMissing(true);
      })
      .catch(() => {});
  ...
  // Auto-join when a Google session exists but no tournament token
  useEffect(() => {
  ...
  }, [tokenReady, userReady, token, user, code, t, fetchState, setTokenFromResponse, isCreator]); // eslint-disable-line react-hooks/exhaustive-deps
      ...
      <main className="flex min-h-screen flex-col bg-zinc-50 dark:bg-zinc-950">
        ...
        <div className="flex flex-1 flex-col items-center justify-center p-6">
          <div className="w-full max-w-sm space-y-6">
            ...
            {!user && <GoogleSignInButton returnTo={`/tournament/${code}`} />}
