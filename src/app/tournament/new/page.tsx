...
import { Trophy, LayoutList, Tag, CheckCircle2, ChevronRight } from "lucide-react";
...
export default function NewTournament() {
  ...
  useEffect(() => {
    if (ready && !user) {
      // Redirect happens via the render branch below; nothing to do here yet.
    }
  }, [ready, user]);
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      if (!response.ok) {
        setError(translateApiError(body.error, t) ?? t.createTournament.somethingWentWrong);
        return;
      }
    ...
    <main className="flex min-h-screen flex-col bg-zinc-50 dark:bg-zinc-950">
      ...
      <div className="flex flex-1 justify-center px-6 py-10">
        <div className="w-full max-w-lg space-y-8">
          ...
          {ready && !user ? (
            <div className="space-y-4 rounded-2xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5">
              <p className="text-sm text-zinc-600 dark:text-zinc-300">
                {t.auth.signInToCreate}
              </p>
              <GoogleSignInButton returnTo="/tournament/new" />
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button
                type="submit"
                disabled={loading || !allRoundNamesFilled || !name.trim()}
